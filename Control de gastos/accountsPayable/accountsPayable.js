...
function obtenerUltimoDiaMes(ano, mesIndex) {
...
function fechaAjustadaDelMes(dia) {
  ...
  const fecha = new Date(anio, mes, diaReal);
...
function pintarAlertas() {
  ...
  pendientes.forEach((d) => {
    ...
    if (faltan > 5) {
      ...
      clase = "alert-warning";
      texto = `?? Pago pronto a ${d.acreedor} (faltan ${faltan} días)`;
    } else if (faltan === 0) {
      clase = "alert-success";
